...
A test of django-cookiecutter on dokku
...
test relies extensively on environment settings which **will not work with Apache/mod_wsgi setups**. It has been deployed successfully with both Gunicorn/Nginx and even uWSGI/Nginx.

For configuration purposes, the following table maps the 'test' environment variables to their Django setting:
...
DJANGO_DEFAULT_FROM_EMAIL               DEFAULT_FROM_EMAIL          n/a                                            "test <noreply@edanweis.com>"
DJANGO_SERVER_EMAIL                     SERVER_EMAIL                n/a                                            "test <noreply@edanweis.com>" 
DJANGO_EMAIL_SUBJECT_PREFIX             EMAIL_SUBJECT_PREFIX        n/a                                            "[test] "
======================================= =========================== ============================================== ======================================================================
...
To run the tests, enter the test/tests directory and run the following commands::
...
..  code-block:: bash
    ...
    git remote add dokku dokku@yourservername.com:test
    git push dokku master
    ssh -t dokku@yourservername.com dokku redis:create test-redis
    ssh -t dokku@yourservername.com dokku redis:link test-redis test
    ssh -t dokku@yourservername.com dokku postgres:create test-postgres
    ssh -t dokku@yourservername.com dokku postgres:link test-postgres test
    ssh -t dokku@yourservername.com dokku config:set test DJANGO_SECRET_KEY=RANDOM_SECRET_KEY_HERE
    ssh -t dokku@yourservername.com dokku config:set test DJANGO_SETTINGS_MODULE='config.settings.production'
    ssh -t dokku@yourservername.com dokku config:set test DJANGO_AWS_ACCESS_KEY_ID=YOUR_AWS_ID_HERE
    ssh -t dokku@yourservername.com dokku config:set test DJANGO_AWS_SECRET_ACCESS_KEY=YOUR_AWS_SECRET_ACCESS_KEY_HERE
    ssh -t dokku@yourservername.com dokku config:set test DJANGO_AWS_STORAGE_BUCKET_NAME=YOUR_AWS_S3_BUCKET_NAME_HERE
    ssh -t dokku@yourservername.com dokku config:set test DJANGO_MAILGUN_API_KEY=YOUR_MAILGUN_API_KEY
    ssh -t dokku@yourservername.com dokku config:set test DJANGO_MAILGUN_SERVER_NAME=YOUR_MAILGUN_SERVER
    ssh -t dokku@yourservername.com dokku run test python manage.py migrate
    ssh -t dokku@yourservername.com dokku run test python manage.py createsuperuser
...
application stacks on one host, remove the port setting entirely and add `VIRTUAL_HOST=edanweis.com` to your env file.
This pass all incoming requests on `nginx-proxy` to the nginx service your application is using.

.. _nginx-proxy: https://github.com/jwilder/nginx-proxy

Postgres is saving its database files to `/data/test/postgres` by default. Change that if you wan't
...
Place it in `/etc/supervisor/conf.d/test.conf` and run::
